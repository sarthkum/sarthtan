...
<title>cHAPTER 4 sTATIC mETHODS</title>
<% session.setAttribute("key","4_4_3");%>
</head>
<div id="menu">
<ul>
...
<td>
<div id="post">
<h3>cHAPTER 4 sTATIC mETHODS</h3>
<p>Static method are those method that does not operate on objects. For example, the pow method of the MATH class is static object </p> 

 

<pre>MATH.pow(x,a);</pre> 

 

<p>Computes the power of as x ki power a or x raised to the power a .</p> 

 

<p>It does not use any  implicit parameters or MATH class level parameter. </p> 

 

<p>In other words you can think of the static method as the methods that does not have a this parameter.(In a non static method the this parameter refers to the implicit parameters)</p> 

 

<p>A static method a class cannot access the id field of class as it is not static but it can access the nextInt field as it is a static fields.</p> 
...
<p>If you have omitted the static then you have to create a instance of HorseUtil class to to invoke the method.</p> 
...
<p>It is legal to call a static method from an object like horse1.getIntId(); is horse1 is of type HorseUtil instead of HorseUtil.getIntId(); . How ever we should prefer to call static method from the class name as it does not depend upon the object for its creation.</p> 
...
<p>When a method does not need access to the object variables as all the methods are provided explicitly.</p> 

 

<p>When a method need to access only static static field of the class like getIntId()</p> 

 

<p>The term "static" has a curious history. At first, the keyword static was introduced in C to denote local variables that dont go away when a block is exited. </p> 

<p>In that context, the term "static" makes sense: The variable stays around and is still there when the block is entered again. Then static got a second meaning in C, to denote global variables and functions that cannot be accessed from other files. The keyword static was simply reused, to avoid introducing a new keyword. <p> 

<p>Finally, C++ reused the keyword for a third, unrelated, interpretation to denote variables and functions that belong to a class but not to any particular object of the class. That is the same meaning the keyword has in Java.</p> 
...
<a href="../chapter4/4_4_2staticconstants.jsp"> << Previous</a>
<a href="../TableOfContentJava.jsp"> TOC </a>
<a href="../chapter4/4_4_4factorymethods.jsp">Next >> </a>
