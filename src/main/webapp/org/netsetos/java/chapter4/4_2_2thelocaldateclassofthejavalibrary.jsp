...
<title>cHAPTER 4 tHE lOCALdATE cLASS OF THE jAVA lIBRARY</title>
<% session.setAttribute("key","4_2_2");%>
</head>
<div id="menu">
<ul>
...
<td>
<div id="post">
<h3>cHAPTER 4 tHE lOCALdATE cLASS OF THE jAVA lIBRARY</h3>
<p>In the previous example we have discussed about Date class . An instance of Date has a state namely a particular point in time. </p> 

 

<p>When you use the date than it is the calculated time in millisecond from a fixed date which is 00:00:00 UTC,January 1,1970 called as epoch. </p> 

 

<p>But Later it is found that Date class is not very usable in manipulating a calendar which human is used to such as "JANUARY 4 1988" This is following the Gregorian calendar which is used in most of the countries. The same point of time would be described differently in Chinese or Hebrew lunar calendar.</p> 

 

<p>Therefore the Language designer decided to separate the concerns of keeping time and attaching names to point in time. Therefore the standard java library contains two separate classes: the Date class which represents a point in time and the LocalDate Class which express days in familiar calendar notation. </p> 

 

<p>Separating time measurement from the  Calendar is a good object-oriented design.</p> 

 

<p>You do not use a constructor to construct object of the LocalDate class . Instead use static factory method. </p> 

 

<pre>LocalDate.now();</pre> 

 

 

<p>You can construct an Object by supplying year month and day.</p> 

 

<pre>LocalDate birthDay = LocalDate.of('1962,12,9');</pre> 

 

<p>Once you have a LocalDate object, you can find out the year, month, and day with the methods getYear, getMonthValue, and getDayOfMonth:</p> 

 

<pre> 

int year = birthDay .getYear(); // 1999 
int month = birthDay .getMonthValue(); // 12 
int day = birthDay .getDayOfMonth(); // 31 

</pre> 

 

<p>This may seem pointless as you have set these value only . So in case if we came across a value which we have not set. The plusDays method yields a new LocalDate with number of days added to the current Date </p> 

 

<pre> 

        LocalDate after50days = birthDay.plusDays(50); 

         day  = after50days.getDayOfMonth(); 

         year = after50days.getYear(); 

         month = after50days.getMonthValue(); 

</pre> 

 

<p>The LocalDate class has encapsulated instance fields to maintain a date. </p> 

 

<p>Actually, the Date class also has methods to get the day, month, and year, called getDay, getMonth, and getYear, but these methods are deprecated. A method is deprecated when a library designer realizes that the method should have never been introduced in the first place.</p> 

<p>These methods were a part of the Date class before the library designers realized that it makes more sense to supply separate classes to deal with calendars. When an earlier set of calendar classes was introduced in Java 1.1, the Date methods were tagged as deprecated. </p> 

<p>You can still use them in your programs, but you will get unsightly compiler warnings if you do. It is a good idea to stay away from using deprecated methods because they may be removed in a future version of the library.</p> 
...
<a href="../chapter4/4_2_1objectsandobjectvariables.jsp"> << Previous</a>
<a href="../TableOfContentJava.jsp"> TOC </a>
<a href="../chapter4/4_2_3mutatorandaccessormethods.jsp">Next >> </a>
